...
public class ConsoleCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            if (args.length == 0) {
                ConfigManager.getMessageYaml().getStringList("COMSOLE-SHOW-COMMAND").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                return;
            }
            ...
            switch (subCommand) {
                case "list":
                    ConfigManager.getMessageYaml().getStringList("LIST").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));

                    break;

                case "makewish":
                    if (args.length == 1) {
                        ConfigManager.getMessageYaml().getStringList("WISH-NULL").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                        return;
                    }
                    ...
                    if (!WishManager.hasWish(wishName)) {
                        ConfigManager.getMessageYaml().getStringList("WISH-NOT-HAVE").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                        return;
                    }
                    ...
                    if (args.length == 3) {
                        ...
                        if (targetPlayer == null) {
                            ConfigManager.getMessageYaml().getStringList("PLAYER-OFFLINE").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                            return;
                        }

                        WishManager.makeWish(targetPlayer, wishName, false);
                        ConfigManager.getMessageYaml().getStringList("DONE").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                    } else ConfigManager.getMessageYaml().getStringList("PLAYER-NULL").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                ...
                case "getamount":
                    ...
                    if (args.length == 2) {
                        ConfigManager.getMessageYaml().getStringList("PLAYER-NULL").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                        return;
                    }
                ...
                case "setamount":
                    ...
                    if (args.length == 3) {
                        ConfigManager.getMessageYaml().getStringList("AMOUNT-NULL").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                        return;
                    }
                ...
                case "setguaranteed":
                    ...
                    if (args.length == 3) {
                        ConfigManager.getMessageYaml().getStringList("GUARANTEED-NULL").forEach(message -> sender.sendMessage(CC.replaceAndTranslate(message, null, null)));
                        return;
                    }
