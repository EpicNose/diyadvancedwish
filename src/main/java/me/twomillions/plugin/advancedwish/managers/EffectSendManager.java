...
public class EffectSendManager {
    ...
    public static void sendEffect(String fileName, Player player, String path, String pathPrefix) {
        ...
        // isCancelled
        if (QuickUtils.callAsyncEffectSendEvent(fileName, player, path, pathPrefix).isCancelled()) return;
        ...
        if (isRecordEffectSend(fileName, path, pathPrefix)) {
            ...
            String finalLogString = logTime + ";" + player.getName() + ";" + player.getUniqueId() + ";" + UnicodeUtils.stringToUnicode(fileName) + ";" + pathPrefix + ";";
